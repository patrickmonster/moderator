...
const permissions = [
  ...
  "user:edit:follows", // 팔로우 정보 변경 - 차단 /
  "whispers:edit", // 귓속말
  "clips:edit",
]; // 클립생성
...
if (access_token) {
  ...
  // 사용자 인증
  location.href = `https://id.twitch.tv/oauth2/authorize?response_type=token&client_id=${oauth_client_id}&redirect_uri=${oauth_redirect_uri}&scope=${permissions.join(
    "%20"
  )}&state=${channel}`;
} else {
  ...
  window.onload = () => {
    ...
    window.token.instance.get("users").then(({ data: { data } }) => {
      ...
      window.client.on("join", (channel, username, self) => {
        if (self)
          window.client.popup(
            document
              .createElement("span")
      ...
      window.client.on("part", (channel, username, self) => {
        if (self)
          window.client.popup(
            document
              .createElement("span")
        ...
        for (const id of ids) {
          ...
          for (const { code, id: _id } of obj[id]) {
            emots
              ...
              if (chat.value.length && chat.value[chat.value.length - 1] != " ")
                chat.value += " ";
