...
if (access_token) {
  ...
  // 사용자 인증
  location.href = `https://id.twitch.tv/kraken/oauth2/authorize?response_type=token&client_id=${oauth_client_id}&redirect_uri=${oauth_redirect_uri}&scope=${permissions.join(
    "%20"
  )}&state=${channel}`;
} else {
  ...
  window.token.scope = window.token.scope || [
  ...
  window.token.instance
    ...
    .then(({ data: { data } }) => {
      if (data.length) {
        ...
        document
          .getElementById("loading_channel")
          .html(
            `<h1>${window.broadcaster.display_name}(${window.broadcaster.login})</h1>`
          ); // 채널
        load_list();
      }
    })
...
function search_view_bot() {
  ...
  const allFunc = async (list, cursor, total) => {
    ...
    if (cursor) list.push(...(await getUserList(cursor, allFunc)));
    return list;
  };
  ...
  const then = (list) => {
    ...
    list.forEach(({ f, i }) => {
      ...
      if (times[time].length >= viewbot_follow) {
        is_viewbot =
          is_viewbot < times[time].length ? times[time].length : is_viewbot;
      }
    });
    if (is_viewbot) {
    ...
    const newList = Object.assign(
      ...
      list.filter(
        (o) =>
          filter.includes(new Date(o.f).format("yyyy-MM-dd HH:mm")) &&
          (is_kor || o.l == o.n)
      )
    ); // 필터에 접촉 / 닉네임 동일
...
async function getUserList(cursor, f) {
  try {
    const { data } = await window.token.instance.get(
      `users/follows?first=100&to_id=${window.broadcaster.id}${
        cursor ? "&after=" + cursor : ""
      }`
    );
    let c;
...
function load_list(cursor) {
  if (!cursor) clear_();
  const end = onDialogue(
    document.createElement("span").html("데이터를 불러오는중...")
  );
...
function list_item({ f, i, l, n }) {
  ...
  tr
    .C("td")
    .html(n == l ? l : `${pad(n, 20)}(${l})`)
    .styles("text-overflow", "ellipsis")
    .styles("cursor", "pointer")
    .addClass("over_word").onclick = function () {
  ...
  td.html(`
    ...
    // 사용자 정보창
    window.open(
      `https://www.twitch.tv/popout/${channel}/viewercard/${this.parentNode.parentNode.data(
        "login"
      )}?popout=`,
      "_blank",
  ...
  tr = ele.C("tr");
  tr.C("td")
    .html("followd :" + new Date(f).format("yyyy-MM-dd(E)HH:mm:ss"))
    .styles("font-size", "0.5em");

  // 사용자 날자 선택
  tr
    .C("td")
    .html('<a title="Start At"><i class="fas fa-long-arrow-alt-right"></i></a>')
    .styles("cursor", "pointer").onclick = function () {
    const day = this.parentNode.parentNode.data("created_at");
    document.getElementById("day_start").value = new Date(day).format(
      "yyyy-MM-dd"
    );
  ...
  tr
    .C("td")
    .html(
      '<a title="End At"><i class="fas fa-long-arrow-alt-right fa-rotate-180"></i></a>'
    )
    .styles("cursor", "pointer").onclick = function () {
    const day = this.parentNode.parentNode.data("created_at");
    document.getElementById("day_end").value = new Date(day).format(
      "yyyy-MM-dd"
    );
  ...
  tr
    .C("td")
    .html('<a title="ban"><i class="fas fa-ban"></i></a>')
    .styles("cursor", "pointer").onclick = function () {
  ...
  tr
    .C("td")
    .html('<a title="unfollow"><i class="fas fa-shield-alt"></i></a>')
    .styles("cursor", "pointer").onclick = function () {
    var table = this.parentNode.parentNode;
  ...
  tr
    .C("td")
    .html('<a title="remove list item"><i class="far fa-times-circle"></i></a>')
    .styles("cursor", "pointer").onclick = function () {
    var table = this.parentNode.parentNode;
...
function list_filter(l) {
  ...
  let s_d = new Date(
    (document.getElementById("day_start").value == ""
      ? new Date("1999-01-01").format("yyyy-MM-dd")
      : document.getElementById("day_start").value) +
      " " +
      (document.getElementById("time_start").value == ""
        ? new Date("1999-01-01 00:00").format("HH:mm")
        : document.getElementById("time_start").value)
  );
  let e_d = new Date(
    (document.getElementById("day_end").value == ""
...
function unfollow_users(l, is_ban) {
  if (
    confirm("사용자" + l.length + "명이 언팔로우 됩니다!\n진행하시겠습니까?") ==
    true
  ) {
