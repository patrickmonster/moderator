...
Element.prototype.data = function () {
  var a = arguments,
    b = a.length;
  if (!(b - 1)) return this.getAttribute("data-" + a[0]);
  else this.setAttribute("data-" + a[0], a[1]);
  return this;
};
window.addStyle = document.addStyle = function (a, b) {
  b = document.head.C("style");
  b.innerHTML = a;
  return b;
};
...
document.C = (ele = "span") => document.createElement(ele);
Element.prototype.C = function (ele = "span") {
  var ele = document.createElement(ele);
  this.appendChild(ele);
  return ele;
};
Element.prototype.on = function (name, f) {
  this[`on${name}`] = f;
  return this;
};
Element.prototype.S = Element.prototype.styles = function (e, f) {
...
Element.prototype.A = function (a, b) {
  this[a] = b;
  return this;
};
...
function isMobile() {
  ...
  const mobilePhones = new RegExp(
    /\b(:?iphone|ipod|ipad|android|blackberry|windows ce|nokia|webos|opera mini|sonyericsson|opera mobi|iemobile)\b/g
  );
 ...
 * @returns 생성함수
 */
function Popup(max = 10) {
  //팝업 관리자
  const list = []; // 팝업 스텍
  const msg_queue = [];
  const logs = [];
  window.addStyle(
    `#popup{position:fixed;right:0;margin:10px !important;bottom: 0%;}#popup *{width:100%;display: inline-block;}`
  );
  function create(element = "test", time = 5) {
    const timeText = document
      .createElement("p")
      .html(new Date().format("hh:mm:ss"));
    if (element == "") return logs;
    else if (time == -1) {
      logs.push(
        `${timeText.innerHTML}${
          typeof element === "string" ? element : element.innerHTML
        }`
      );
      return;
    }
    ...
    popup.styles("bottom", `${popup.pos_value * -1}px`).onclick = function () {
      end(this);
    };
    popup.loop = () => {
      popup.styles(
        "bottom",
        `${(popup.pos_value -= popup.list_index + 1) * -1}px`
      );
    ...
    const loop = () => {
      ...
      if (popup.pos_value <= max) window.requestAnimationFrame(loop);
        ...
        if (msg_queue.length) {
          ...
          element
            .styles("z-index", 9999 - element.list_index)
            .styles("opacity", "1");
          element.loop();
...
function targetPopup(element, id = "popup2") {
  ...
  return popup
    ...
    .appendChild(element.styles("width", "300px").styles("height", "auto"));
}
